...
unsigned long saved_timestamp;      // Timestamp stored in LittleFS
...
//  Serial.print (" Minute(s).");
  ESP.deepSleep(10 * 60e6); // convert to microseconds
} // end of goToSleep()
...
void setup() {
 pinMode(D8,OUTPUT);
  ...
  // Voltage divider R1 = 100k+220k =320k and R2=100k
  float calib_factor = 4.1; // change this value to calibrate the battery voltage
  ...
  //*****************Checking if LittleFS available********************************

  Serial.println("LittleFS Initialization: (First time run can last up to 30 sec - be patient)");
  ...
  ReadFromLittleFS();              //read stored values and update data if more recent data is available
  ...
  if (accuracy < 12) {
    ...
    WriteToLittleFS(current_timestamp);                   // update timestamp on storage
  }
  else {         
    WriteToLittleFS(saved_timestamp);                     // do not update timestamp on storage
  }
...
//*******************************************************************************
  ...
  char _current_timestamp[8];
  dtostrf(current_timestamp,3 ,1, _current_timestamp);

  client.publish("home/weather/solarweatherstation/time", _current_timestamp, 1);      // ,1 = retained
  delay(50);

 char _DHTtempC[8];                                // Buffer big enough for 7-character float
  dtostrf(DHTtempC, 3, 1, _DHTtempC);               //
  client.publish("home/weather/solarweatherstation/DHTtempC", _DHTtempC, 1);     // ,1 = retained
  delay(50);
  char _DHTtempF[8];                                // Buffer big enough for 7-character float
  dtostrf(DHTtempF, 3, 1, _DHTtempF);               //
  client.publish("home/weather/solarweatherstation/DHTtempF", _DHTtempF, 1);     // ,1 = retained
  delay(50);
  char _DHThumi[8];                                // Buffer big enough for 7-character float
  dtostrf(DHThumi, 3, 1, _DHThumi);               //
  client.publish("home/weather/solarweatherstation/DHThumi", _DHThumi, 1);     // ,1 = retained
  delay(50);  
  ...
  delay(50);
   ...
   char _adjusted_tempf[8];                                // Buffer big enough for 7-character float
  dtostrf(adjusted_tempf, 3, 1, _adjusted_tempf);               // Leave room for too large numbers!

  client.publish("home/weather/solarweatherstation/tempf", _adjusted_tempf, 1);      // ,1 = retained
  delay(50);
  ...
  dtostrf(measured_presHg, 3, 2, _measured_pres);               // Leave room for too large numbers!
  ...
  dtostrf(rel_pressure_roundedHg, 3, 2, _rel_pressure_rounded);               // Leave room for too large numbers!
  ...
  char _DewpointTemperatureF[8];                                // Buffer big enough for 7-character float
  dtostrf(DewpointTemperaturef, 3, 1, _DewpointTemperatureF);               // Leave room for too large numbers!

  client.publish("home/weather/solarweatherstation/dewpointF", _DewpointTemperatureF, 1);      // ,1 = retained
  delay(50);

  char _HeatIndex[8];                                // Buffer big enough for 7-character float
  dtostrf(HeatIndexf, 3, 1, _HeatIndex);               // Leave room for too large numbers!
...
void checkForUpdates()
{
  ...
  t_httpUpdate_return ret = ESPhttpUpdate.update(espClient, checkUrl );
...
void measurementEvent() { 
 ...
 delay(dht.getMinimumSamplingPeriod());
  ...
  DHTtempC = dht.getTemperature()+0.0;
  DHTtempF = (1.8*DHTtempC)+32 ;   
delay(2000);
 DHThumi = dht.getHumidity();
  
  bme.takeForcedMeasurement();

  // Get temperature
  measured_temp = bme.readTemperature();
  measured_tempf = (1.8*measured_temp)+32;
...
//   print on serial monitor
  ...
  measured_pres = bme.readPressure() / 100.0F;
   measured_presHg = (bme.readPressure() / 100.0F) / 33.8;
  // print on serial monitor
  ...
  SLpressure_Hg = SLpressure_hPa / 33.8;
  rel_pressure_roundedHg = (rel_pressure_rounded /33.8) + 0.12;
  // print on serial monitor
  ...
  adjusted_tempf= (adjusted_temp *1.8)+32;
  // print on serial monitor
...
void ReadFromLittleFS() {
  ...
  File myDataFile = LittleFS.open(filename, "r");       // Open file for reading
  if (!myDataFile) {
    Serial.println("Failed to open file");
    FirstTimeRun();                                   // no file there -> initializing
  }
  
  Serial.println("---> Now reading from LittleFS");
  
  String temp_data;
  
  temp_data = myDataFile.readStringUntil('\n');  
  saved_timestamp = temp_data.toInt();
  Serial.print("Timestamp from LittleFS: ");  Serial.println(saved_timestamp);
  
  temp_data = myDataFile.readStringUntil('\n');  
  accuracy = temp_data.toInt();
  Serial.print("Accuracy value read from LittleFS: ");  Serial.println(accuracy);
...
void WriteToLittleFS(int write_timestamp) {
  char filename [] = "/data.txt";
  ...
  Serial.println("---> Now writing to LittleFS");
  ...
  myDataFile = LittleFS.open(filename, "r");             // Open file for reading
...
void FirstTimeRun(){
  ...
  accuracy = 1;
  char filename [] = "/data.txt";
  File myDataFile = LittleFS.open(filename, "w");            // Open a file for writing
  if (!myDataFile) {
    Serial.println("Failed to open file");
